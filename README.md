...
Для заданного графа в худшем случае сложность определяется операциями над очередью с приоритетом: получаем `O(|V|)` вставок в очередь и такая же для получений. Таким образом получаем сложность `O(|V|*log(|V|))`.
